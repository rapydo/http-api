...
if [[ "$TEMPLATE" == "postgres" ]]; then
  rapydo create prj --auth postgres --frontend no -e AUTH_TOKEN_IP_GRACE_PERIOD=1
elif [[ "$TEMPLATE" == "mysql" ]]; then
  rapydo create prj --auth mysql --frontend no -e AUTH_TOKEN_IP_GRACE_PERIOD=1
elif [[ "$TEMPLATE" == "neo4j" ]]; then
  rapydo create prj --auth neo4j --frontend no -e AUTH_TOKEN_IP_GRACE_PERIOD=1
elif [[ "$TEMPLATE" == "mongo" ]]; then
  rapydo create prj --auth mongo --frontend no -e AUTH_TOKEN_IP_GRACE_PERIOD=1
  ...
  rapydo create prj --current --extend prjbase -s ftp -s pushpin -s bot --auth ${AUTH} --frontend no --add-optionals -e AUTH_FORCE_FIRST_PASSWORD_CHANGE=1 -e AUTH_MAX_PASSWORD_VALIDITY=60 -e AUTH_DISABLE_UNUSED_CREDENTIALS_AFTER=60 -e AUTH_MAX_LOGIN_ATTEMPTS=10 -e AUTH_LOGIN_BAN_TIME=10 -e AUTH_SECOND_FACTOR_AUTHENTICATION=1 -e AUTH_TOKEN_IP_GRACE_PERIOD=1
