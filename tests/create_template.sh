...
if [[ "$TEMPLATE" == "postgres" ]]; then
  ...
  rapydo create prj --auth postgres --frontend no -e AUTH_SECOND_FACTOR_AUTHENTICATION=1
elif [[ "$TEMPLATE" == "mysql" ]]; then
  # TOTP will be checked and disabled because AUTH_FORCE_FIRST_PASSWORD_CHANGE is 0;
  rapydo create prj --auth mysql --frontend no -e AUTH_SECOND_FACTOR_AUTHENTICATION=1
elif [[ "$TEMPLATE" == "neo4j" ]]; then
  # TOTP will be checked and disabled because AUTH_FORCE_FIRST_PASSWORD_CHANGE is 0;
  rapydo create prj --auth neo4j --frontend no -e AUTH_SECOND_FACTOR_AUTHENTICATION=1
elif [[ "$TEMPLATE" == "mongo" ]]; then
  # TOTP will be checked and disabled because AUTH_FORCE_FIRST_PASSWORD_CHANGE is 0;
  rapydo create prj --auth mongo --frontend no -e AUTH_SECOND_FACTOR_AUTHENTICATION=1
